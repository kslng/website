...
The objective of the using keyword is to enable multi-file projects to be
compiled without manually defining all included files from the command line, it
also serves as a way to disclose which existing object files should be linked
after codegen.
...
Following a using keyword with a string will tell the compiler to include
another source file at the path provided, relative paths are preferred, example:
...
The second way to use the using keyword is with identifiers. This syntax will
tell the compiler to link an object file following the path of the identifier.
For example:
